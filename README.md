...
This repository has the sole-purpose of running Wan2.1 on Mac M-Series chips efficiently. Distributed inference and various things are broken, but the memory savings are real.

## Original problems

Mac M-Series chips have Unified Memory, so original method of offload model to CPU still uses memory.

Original repo also loads all models at the start, taking up a lot of memory. (umt5-xxl is a 13B model!!)

## Changes

- Load models only when needed. (T5, base model, and vae)
- Change the offload_model method to delete the model from memory immediately after use.
